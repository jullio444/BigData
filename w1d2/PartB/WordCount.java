...
public class WordCount {
	...
	WordCount(){
		mappers = new ArrayList<>(Arrays.asList(
				                          new Mapper("//src//bigData//w1d2//PartB//textfile/testData1.txt"),
				                          new Mapper("//src//bigData//w1d2//PartB//textfile/testData2.txt"),
				                          new Mapper("//src//bigData//w1d2//PartB//textfile/testData3.txt")));
		reducers = new ArrayList<>(Arrays.asList(new Reducer(),
				                                 new Reducer(),
				                                 new Reducer(),
				                                 new Reducer()));
		m = mappers.size();
	...
	public List<List<Pair<String, Integer>>> jobTracker() {
		...
		List<Pair<String, Integer>> mapperOutput1 = mappers.get(0)
				.createListPairs(mappers.get(0).readFromFile());
		mapperOutputs.add(mapperOutput1);
		System.out.println("Mapper 0 Output");
		mapperOutput1.forEach(System.out::print);
		System.out.println();
		
		List<Pair<String, Integer>> mapperOutput2 = mappers.get(1)
				.createListPairs(mappers.get(1).readFromFile());
		mapperOutputs.add(mapperOutput2);
		System.out.println("Mapper 1 Output");
		mapperOutput2.forEach(System.out::print);
		System.out.println();

		List<Pair<String, Integer>> mapperOutput3 = mappers.get(2)
				.createListPairs(mappers.get(2).readFromFile());
	...
	public static void main(String[] args) {
		...
		w.jobTracker();
		w.pairsSentToReducers();
		List<List<Pair<String, Integer>>> listOfPairsSentToReducers = w.pairsSentToReducers();
		...
		for(List<Pair<String, Integer>> lp: listOfPairsSentToReducers) {
			List<GroupByPair<String, List<Integer>>> reducerInput= new ArrayList<>(w.reducers.get(listOfPairsSentToReducers
					.indexOf(lp)).generateReducerInput(lp));
		...
		for(List<GroupByPair<String, List<Integer>>> gp: reducerInputs) {
			List<GroupByPair<String, Integer>> reducerOutput= new ArrayList<>(w.reducers.get(reducerInputs
					.indexOf(gp)).generateReducerOutput(gp));
